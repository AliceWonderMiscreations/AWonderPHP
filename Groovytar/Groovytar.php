...
abstract class Groovytar
{
    ...
    {
        $salts = array();
        $salts[]=\AWonderPHP\PluggableUnplugged\Misc::saltShaker();
        $salts[]=\AWonderPHP\PluggableUnplugged\Misc::saltShaker();
        return $salts;
    }//end generateSalts()
    ...
    public function addDomain(string $input): array
    {
        ...
        $domain=\AWonderPHP\PluggableUnplugged\PunycodeStatic::punycodeDomain($domain);
        $test='user@' . $domain;
        if (! filter_var($test, FILTER_VALIDATE_EMAIL)) {
            throw \AWonderPHP\Exceptions\InvalidArgumentException::invalidDomain($input);
        }
    ...
    public function addEmailAddress($input): array
    {
        ...
        $paddress=\AWonderPHP\PluggableUnplugged\PunycodeStatic::punycodeEmail($address);
        if (! filter_var($paddress, FILTER_VALIDATE_EMAIL)) {
            throw \AWonderPHP\Exceptions\InvalidArgumentException::invalidEmail($input);
        }
    ...
    protected function mimicHash($email): string
    {
        ...
        $pemail=\AWonderPHP\PluggableUnplugged\PunycodeStatic::punycodeEmail($email);
        //validate email
    ...
    public function referenceHash($email)
    {
        ...
        //validate email
        if (! filter_var($pemail, FILTER_VALIDATE_EMAIL)) { //hopefully wordpress already has validated this but...
            throw \AWonderPHP\Exceptions\InvalidArgumentException::invalidEmail($email);
        }
        $prehash = $pemail;
        for ($i=0; $i<5; $i++) {
    ...
    public function getAvatarData($id_or_email, $args = null)
    {
        ...
        $args['url'] = \AWonderPHP\PluggableUnplugged\WPCoreReplace::modifyQueryArgs($url, $url_args);
        return $args;
    }//end getAvatarData()
